...
class HomeController extends Controller
{
    ...
    public function showregistrationpage(){
        ...
        $category = Category::select('*')->where('parent_id',0)->where('status',1)->get();
        return view('admin-views.store-registration',compact('category'));
    }
    public function saveStoreRegistration(Request $request)
    {
        ...
        $rules = [
            ...
            'mob' => 'required|string|max:15|unique:stores,phone',
            'email' => 'required|email|unique:stores,email',
            'pwd' => 'required|string|min:8',
            'storeLogo' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
            'storeBanner' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
        ];

        // Custom validation messages
        $messages = [
            'mob.unique' => 'The mobile number already exists in the database.',
            'email.unique' => 'The email entered already exists in the database.',
        ];
        ...
        // Validation has passed; you can access the validated data using $request->all()

        // Handle image uploads
        // if ($request->hasFile('storeLogo')) {
        //     $storeLogoPath = $request->file('storeLogo')->store('store_logos', 'public');
        //     // Save $storeLogoPath to the database or perform other actions
        // }

        // if ($request->hasFile('storeBanner')) {
        //     $storeBannerPath = $request->file('storeBanner')->store('store_banners', 'public');
        //     // Save $storeBannerPath to the database or perform other actions
        // }
      
        $vendor = new Vendor();
        $vendor->f_name = $request->ownername;
        $vendor->l_name = "";
        $vendor->email = $request->email;
        $vendor->phone = $request->mob;
        $vendor->password = bcrypt($request->pwd);
        $vendor->status = null;
        $vendor->save();

        $store = new Store;
        $store->name = $request->storeName;
        $store->phone = $request->mob;
        $store->email = $request->email;
        $store->city_id = '1';
        $store->area_id = '1';
        $store->store_category = $request->cat;
        $store->logo = Helpers::upload('store/', 'png', $request->file('storeLogo'));
        $store->cover_photo = Helpers::upload('store/cover/', 'png', $request->file('storeBanner'));
        ...
        // Example:
        return redirect()->route('showregistrationpage')->with('success', 'Store registration saved successfully');
    }
}
